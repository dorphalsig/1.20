...
pub fn generate_pcm_fixtures() -> std::io::Result<()> {
...
pub fn ensure_pcm_fixtures() -> std::io::Result<()> {
    let out_dir = Path::new("integration_test/assets/pcm");
    let mut missing = false;
    for (_, output_name, _) in TARGETS {
        if !out_dir.join(output_name).exists() {
            missing = true;
            break;
        }
    }
    if missing {
        generate_pcm_fixtures()?;
    }
    Ok(())
}
